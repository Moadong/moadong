...
- **React Compiler + nullable 접근**: 로딩 중 `null`일 수 있는 값(쿼리/SSE 상태 등)의
  프로퍼티를 `x!.prop`(논-널 단언)이나 무가드 `x.prop`로 접근하면 지적한다.
  소스에 `if(!x) return` 가드가 있어도 컴파일러가 의존성 읽기를 그 위로 호이스팅해
  렌더 중 크래시할 수 있으니, 접근 지점을 옵셔널 체이닝(`x?.prop`)으로 쓰게 한다.
