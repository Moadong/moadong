...
3. 입력받은 키로 **Jira API를 호출**하여 스토리 정보를 자동 조회합니다:
...
4. 조회된 정보를 기반으로 **자동 생성**합니다:
   - **제목**: Jira 스토리 제목과 동일하게 사용
   - **담당자**: git config user.name으로 자동 설정
   - **브랜치명**: 제목을 영문 kebab-case로 변환하여 `feature/#이슈번호-<설명>-<MOA키>` 형식으로 생성 (이슈번호는 아직 모르므로 적절히 작성)
...
5. 자동 생성된 내용을 **사용자에게 보여주고 확인**받습니다:
...
📋 자동 생성 결과:
  ...
  담당자: suhyun113
  상위 스토리: MOA-874
  브랜치명: feature/#이슈번호-jira-story-script-improve-MOA-874
  분기 브랜치: develop-fe
  라벨: 🔨 Refactor
  태스크:
...
```bash
./scripts/jira-task.sh "제목" "담당자" "MOA-xxx" "브랜치명" "분기브랜치" "" "태스크" "라벨"
```
...
- 브랜치명은 제목의 핵심 키워드를 영문 kebab-case로 변환하여 생성합니다.
