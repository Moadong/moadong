...
4. `src/styles/theme.test/index.ts`에 새 보류 토큰이 생겼으면 디자이너에게 "새 토큰인지, 기존 토큰 오기입인지" 확인을 요청한다. 컨펌된 값만 `src/styles/theme/`으로 옮기고 `theme.test`에서 지운다. 이 파일은 스크립트가 덮어쓰므로 손으로 고치지 않는다.
...
| 레이아웃 (자식 간격·여백) ±0.5px | 예 | 루트의 직계 자식 사이 간격과 앞뒤 여백. `gap`·`padding` 차이가 여기서 잡힌다 |
...
- Figma 텍스트에 검정 20% 같은 반투명 fill이 겹쳐 있으면 토큰이 아니라 시안 정리 대상이다. 리포트의 "반투명 겹침 fill" 표에 따로 나오고 theme.test에는 안 들어간다.
...
- **내부 간격 차이는 루트 크기로는 안 보인다.** 자식이 늘어나 흡수하면 바깥 크기가 그대로라 크기 축은 PASS다. 레이아웃 축이 그걸 잡는다. 반대로 레이아웃 축은 **직계 자식까지만** 본다 — 더 안쪽의 간격 차이는 여전히 안 보인다.
