...
3. **인터페이스 설계**
   - 네이티브 HTML 요소 래핑 시: `extends HTMLButtonAttributes<HTMLButtonElement>` 등 확장
...
네이티브 HTML 요소를 래핑할 때는 해당 요소의 Attributes를 extend해 모든 HTML 속성(`aria-*`, `data-*`, 이벤트 핸들러 등)을 자동으로 지원:
...
const useDropDownContext = () => {
  ...
  if (!context) throw new Error('Provider 내부에서만 사용 가능');
  return context;
};
...
- `src/components/common/Button/Button.tsx` - 기본 컴포넌트 패턴 (HTMLAttributes extend)
