---
name: "rn"
description: "모아동 React Native + Expo 프로젝트와 프론트엔드 WebView 연동을 작업할 때 사용한다. RN, React Native, Expo, WebView 브릿지, 딥링크, 푸시 알림, 앱 라우팅, /RN 요청에 사용한다."
---
...
`사용자의 현재 요청`가 비어 있으면, 사용자에게 어떤 RN 관련 작업이 필요한지 물어본 뒤 RN에이전트를 호출하세요.

Agent 툴로 `RN에이전트` 서브에이전트를 호출하여 위 작업을 처리하세요.

다음 컨텍스트를 에이전트에게 전달하세요:

- RN 프로젝트 경로: `../moadong-react-native/`
- 현재 프론트엔드 프로젝트 경로: `./`
- 두 프로젝트가 WebView로 연동되어 있음 (프론트엔드 웹 → RN WebView)

---

## Source: `.claude/agents/RN에이전트.md`
