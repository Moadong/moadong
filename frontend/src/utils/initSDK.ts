...
export function initializeMixpanel() {
  if (!import.meta.env.VITE_MIXPANEL_TOKEN) {
    logger.warn('믹스패널 환경변수 설정이 안 되어 있습니다.');
    return;
  }
...
export function initializeSentry() {
  ...
  if (!import.meta.env.VITE_SENTRY_DSN) {
    logger.warn('Sentry DSN이 설정되지 않았습니다.');
    return;
  }
...
export function initializeKakaoSDK() {
  if (!import.meta.env.VITE_KAKAO_JAVASCRIPT_KEY) {
    logger.warn('환경변수가 설정되어 있지 않습니다.');
    return;
  }

  if (!window.Kakao) {
    logger.error('카카오 SDK가 로드되지 않았습니다.');
    return;
  }
  ...
  try {
    ...
    logger.error('카카오 SDK 초기화에 실패했습니다:', error);
  }
}
