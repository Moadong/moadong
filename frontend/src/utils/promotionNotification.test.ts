...
describe('promotionNotification 유틸 함수 테스트', () => {
  ...
  describe('getLatestPromotionTime 함수', () => {
    ...
    it('ID가 유효하지 않으면 eventStartDate를 기준으로 시간을 반환해야 한다', () => {
      ...
      expect(getLatestPromotionTime(mockArticles)).toBe(new Date(dateStr).getTime());
    });

    it('여러 개의 글 중 가장 최근 시간을 반환해야 한다', () => {
      const mockArticles = [
        { id: '600000000000000000000000', eventStartDate: '2021-01-01T00:00:00Z' },
        { id: '650000000000000000000000', eventStartDate: '2023-01-01T00:00:00Z' }
      ] as PromotionArticle[];
