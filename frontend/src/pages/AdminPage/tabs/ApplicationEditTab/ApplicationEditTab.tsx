...
const ApplicationEditTab = () => {
  ...
  const {mutate: createMutate, isPending: isCreating} = useMutation({
    mutationFn: (payload: ApplicationFormData) => createApplication(payload, clubId),
    onSuccess: () => {
      queryClient.invalidateQueries({queryKey: ['allApplicationForms', clubId]});
      alert('지원서가 성공적으로 생성되었습니다.');
      navigate(`/admin/${clubId}/applications`);
    },
