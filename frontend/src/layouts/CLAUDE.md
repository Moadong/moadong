...
- **바텀네비**: `src/components/common/BottomNavigation/` (홈/동아리/홍보/메뉴). 두 번째 탭은 `AppLayout`이 `useClubListPath()`로 정해 `showClubsTab` prop으로 내린다. `/clubs`면 동아리, 아니면 구독이다(개편을 못 받은 control은 홈이 곧 목록이라 동아리 탭이 필요 없고, 구독 진입점이 헤더로 옮겨진 건 개편 홈뿐이라 구독 탭을 유지한다). 바텀네비가 직접 훅을 부르면 데스크톱 폭·실험 미배정인 Storybook에서 항상 구독 탭만 보인다. 상세·폼·관리자 등 AppLayout 밖 페이지에는 미노출.
