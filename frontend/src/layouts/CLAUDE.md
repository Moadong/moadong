...
- **바텀네비**: `src/components/common/BottomNavigation/` (홈/동아리/홍보/메뉴). 동아리 탭은 `useClubListPath()`가 `/clubs`일 때만 노출한다(개편을 못 받은 control은 홈이 곧 목록이라 탭을 뺀다). 상세·폼·관리자 등 AppLayout 밖 페이지에는 미노출.
